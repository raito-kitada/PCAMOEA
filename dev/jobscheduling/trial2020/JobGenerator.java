...
public class JobGenerator {
	...
	private double arrive_time_base = 0.0;
	...
	public JobGenerator(int max_node) {
		...
		try {
			BufferedReader br = new BufferedReader(new FileReader("./jobscheduling/trial2020/ProbabilityDistribution_small.csv"));
...
//		}
	...
	private double getArriveInterval(double lambda) {
		double r = PRNG.nextFloat();
		return -(1.0/lambda)*Math.log(1 - r);
	}
	
	public double getArriveTimeBase() {
		return arrive_time_base;
	}
	
	public void setArriveTimeBase(double arrive_time_base) {
		this.arrive_time_base = arrive_time_base;
	}
	...
	public Job nextJob() {
		...
		double arrive_interval = getArriveInterval(0.812); // lambda = 0.00812, [sec]
		double arrive_time = (arrive_time_base + arrive_interval) / 3600; // [hour]
		arrive_time_base += arrive_interval;
		Job job = new Job(request_node_num, request_time, arrive_time);
