...
int main(int argc, char* argv[]) {
	...
	/* convert char* argv[] into an string with proper escaping */
	int len, quote;
	int offset = 0;
	...
	for (i=optind; i<argc; i++) {
		...
		for (j=0; j<len; j++) {
			if ((argv[i][j] == ' ') || (argv[i][j] == '\t') || (argv[i][j] == '\"') || (argv[i][j] == '%') || (argv[i][j] == '|') || (argv[i][j] == '(') || (argv[i][j] == ')')) {
				quote = 1;
				break;
			}
		}
		...
		for (j=0; j<len; j++) {
			...
			buffer[offset++] = argv[i][j];
		}

		if (quote && (argv[i][len-1] == '\\')) {
			buffer[offset++] = '\\';
		}
